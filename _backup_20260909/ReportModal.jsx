...
export default function ReportModal({
  ...
  const handleSaveReport = async () => {
    await saveReport({
      ...
      drStage: screeningData.drStage,
      drStageLabel: screeningData.drStageLabel || 'Stage 2 — Moderate',
      status: 'Finalized',
      secondaryFindingsSummary: screeningData.findingsSummary || 'Screened for multi-disease retinal conditions',
      primaryFinding: screeningData.primaryFinding || screeningData.drStageLabel,
      recommendedNextStep: screeningData.recommendedNextStep || 'Further evaluation by an eye-care professional is recommended.'
    });
    setIsSaved(true);
  };

  const secondaryFindings = screeningData.secondaryFindings || [
    { label: 'AMD / ARMD', status: 'Possible finding' },
    { label: 'BRVO', status: 'No significant finding detected' },
    { label: 'ODC', status: 'No significant finding detected' },
    { label: 'Glaucoma', status: 'Needs review' },
    { label: 'Cataract', status: 'No significant finding detected' },
    { label: 'Myopia', status: 'No significant finding detected' },
    { label: 'Other', status: 'No significant finding detected' }
  ];
            ...
            <div className="report-section-block">
              ...
              <div className="dr-result-banner">
                <div className="dr-result-main">
                  <span className="dr-stage-tag">{screeningData.drStageLabel || 'Stage 2 — Moderate'}</span>
                  ...
                  <tbody>
                    {secondaryFindings.map((f, i) => (
                      <tr key={i}>
                        ...
                        <td>
                          <span className={`table-status-pill ${f.status.includes('No') ? 'pill-safe' : f.status.includes('review') ? 'pill-review' : 'pill-warn'}`}>
                            {f.status}
                          </span>
                        </td>
                      </tr>
            ...
            <div className="report-section-block summary-block">
              ...
              <div className="summary-list">
                ...
                <div className="summary-item">
                  ...
                  <span className="summary-val">{screeningData.findingsSummary || 'AMD / ARMD: Possible finding'}</span>
                </div>
